...
```
docker run -d --name server-postgres -e POSTGRES_DB=db_drupal -e POSTGRES_PASSWORD=1701 -e POSTGRES_USER=user_drupal --network net-drupal postgres
```
...
```
docker run -d --name client-postgres --publish published=9500,target=80 -e PGADMIN_DEFAULT_PASSWORD=54321 -e PGADMIN_DEFAULT_EMAIL=mateo.espinoza@epn.edu.ec --network net-drupal dpage/pgadmin4
```
...
```
```
docker run -d --name server-drupal --publish published=9700,target=80 -v vol-drupal-config:/var/www/html/sites/default/config -v vol-drupal-modules:/var/www/html/modules -v vol-drupal-themes:/var/www/html/themes -v vol-drupal-files:/var/www/html/sites/default/files --network net-drupal drupal
```

### Ingrese al server-drupal y siga el paso a paso para la instalación.
![Volúmenes](imagenes/DrupalP4.jpeg)

_La instalación puede tomar varios minutos, mientras espera realice un diagrama de los contenedores que ha creado en este apartado._

![Volúmenes](imagenes/DiagramaDrupal.jpeg)
